...
class AttnGRU:
    ...
    def __init__(self, num_units, is_training, bn):
        self._num_units = num_units #number of units in the GRU
        self.is_training = is_training
        self.batch_normx = bn
  
 
    def __call__(self, inputs, state, attention, scope=None): #this can run with the object we created 
...
#attention is the new update gate..
        ...
        with tf.variable_scope(scope or 'AttrGRU'):
            with tf.variable_scope("Gates"):  # Reset gate and update gate.
                ...
                r = tf.nn.sigmoid(self._linear(inputs, state, bias_default=1.0))  #the reset gate the in modified GRU
            with tf.variable_scope("Candidate"):
                c = tanh(self._linear(inputs, r * state))  #intermidiate state 

            new_h = attention * c + (1 - attention) * state #this is the new hidden state 
        return new_h      #return the output function 

    def _linear(self, x, h, bias_default=0.0):  #this is to multiply the internal things with gates 
